...
class NoArvore: #esta classe representa a arvore; e um no individual da arvore
    ...
    def procurar_nivel(self): #conta se o numero de antepassados (pais anteriores)
        ...
        p = self.pai
        while p: #enquanto existir pai incrementa-se um no nivel
           nivel += 1
           p = p.pai
        return nivel

    def mostrar_arvore(self): #funcao para imprimir a arvore
      espacos = ' ' * self.procurar_nivel() * 3 #isto inicializa um espaco a cada nivel; nivel 1 com 1 espaco, 2 com 2 e etc..
      ramo = espacos + "|_" if self.pai else "" #se existir pai
      print(ramo + self.valor) #imprime o pai
...
def build_product_tree():
    raiz = NoArvore("Electronics")

    laptop = NoArvore("Laptop")
